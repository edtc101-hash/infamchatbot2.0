...
- ★ 각 문장 사이에 반드시 빈 줄(줄바꿈 2번)을 넣어 가독성을 확보하세요. 고객이 카카오톡/문자에서 한눈에 읽을 수 있도록 문단을 확실히 나누세요.
- ★ "안녕하세요, 인팸입니다" 등의 형식적 인사말은 절대 사용하지 마세요. 바로 본론으로 시작하세요.
- 이모지를 적절히 활용하여 가독성 좋게 작성하세요.
...
app.get('/api/manuals/:filename/summary', async (req, res) => {
    try {
        ...
        const summaryPrompt = `다음은 회사 내부 업무 매뉴얼입니다. **시각적으로 보기 좋고 조직적인 요약**을 만들어주세요.

📐 **출력 형식 규칙 (반드시 따를 것):**

1. **최상단에 한 줄 요약** — 이 매뉴얼이 무엇인지 1줄로 설명
2. **섹션별로 이모티콘 헤더** 사용:
   - 📋 개요, 🔧 업무 절차, ⚠️ 주의사항, 💡 핵심 팁, 📊 수치/기준 등
   - 각 섹션 사이에 반드시 \`---\` 구분선을 넣어
3. **핵심 수치/비율이 있으면 표(table)** 로 정리:
   - \`| 항목 | 값 |\` 형식
4. **단계별 절차는 번호 + 체크박스 스타일**:
   - \`1. ✅ 첫 번째 단계\`
   - \`2. ✅ 두 번째 단계\`
5. **중요 주의사항**은 \`⚠️\`로, 꿀팁은 \`💡\`로 표시
6. **핵심 키워드**는 **굵게** 표시
7. 원본의 **50~70%** 수준으로 압축하되, 정보 손실 없도록
8. 불필요한 인사말, 강의 멘트는 완전 제거
9. 마크다운 형식으로 작성
